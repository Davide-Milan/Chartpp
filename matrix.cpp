...
// old with polymorphism instead of template Data
...
void Matrix::addColumnMatrix(bool numeric)
{
    if(numeric){
        if(dataMatrix.isEmpty())
            dataMatrix.append(new QVector<Data<double>*>(1, new Data)); //adds new QVector<Data*> with 1 default Data
        else{
            dataMatrix.append(new QVector<Data*>(dataMatrix[0]->size(), new Data<double>)); //adds new QVector<Data*> with default Datas, as many as matrix heigth
        }
    }
    else{
        if(dataMatrix.isEmpty())
...
void Matrix::deleteRowMatrix(unsigned int row)
{
    for(unsigned int x = 0; x < getDataMatrixWidth(); x++){
        ...
        dataMatrix[x]->erase(dataMatrix[x]->begin() + row);
...
QVector<QVector<Data *>*>* Matrix::getData()
{
    return &dataMatrix;
}
